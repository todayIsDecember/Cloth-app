    ...
    case 'product':
      return (
        <div className={cn(className, styles.modal, {[styles.open]: isActive})} {...props}>
          <Link href={`/autorization/admin/productsPanel/product_${id_el}`}><Button appearance="white" size="l">Редагувати</Button></Link>
        </div>
      )
    case 'reviews':
      return (
        <div className={cn(className, styles.modal, {[styles.open]: isActive})} {...props}>
          <Button appearance="white" size="l" onClick={() => sendStatus? sendStatus(true): null}>Видалити</Button>
        </div>
      )
    case 'price':
      return (
